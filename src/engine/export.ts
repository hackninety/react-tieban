...
export function chartToMarkdown(chart: Chart, resolve: VerseResolver, opts?: { now?: string }): string {
  ...
  L.push('## 流年条文（1–100 岁）', '');
  L.push('三口径互为参照：**原条文**＝该岁「流年字母×岁数」直查 14-14（基数＋加数，即公式列）；');
  L.push('**校正后**＝原行校正数按岁段换算（1–10 与 81–108 岁 +2 逢六减六，其余 +3 逢廿减廿）后反查 14-14，上游注「原断语不准时参考」；');
  L.push('**终局**＝原条文号＋刻干数×48（铁板核心公式，按八刻错开 48 阶）。', '');
  L.push('| 岁 | 年份 | 干支 | 四声 | 标记 | 字母 | 校正 | 公式 | 原条文断语 | 校正后断语 | 终局断语 |');
  L.push('|----|------|------|------|------|------|------|------|------------|------------|----------|');
  for (const r of chart.liunian.slice(0, 100)) {
    L.push(`| ${r.age} | ${r.year} | ${r.ganzhi} | ${r.sound} | ${r.marker} | ${r.letter === '?' ? '—' : r.letter} | ` +
